...
vector<int> parseInts(string str) {
    stringstream s(str); //used for seperation of elements
    vector<int> value; //used to store result
    char ch; //using this  to store commas
    int elem; //used to store string elements

    while (s >> elem) {
        value.push_back(elem); //inserting elements in string
        s >> ch; //extracting commas
    }
